...
function CyclingTags({ tags }: { tags: string[] }) {
  const [active, setActive] = useState(0);
  const [paused, setPaused] = useState(false);

  useEffect(() => {
    if (paused) return;
    const t = setInterval(() => setActive(i => (i + 1) % tags.length), 2200);
    ...
    <div className="space-y-4">
      ...
      <div className="relative h-12 overflow-hidden">
        {tags.map((tag, i) => (
          <motion.span
            key={tag}
            animate={i === active ? { opacity: 1, y: 0 } : { opacity: 0, y: i === (active - 1 + tags.length) % tags.length ? -24 : 24 }}
