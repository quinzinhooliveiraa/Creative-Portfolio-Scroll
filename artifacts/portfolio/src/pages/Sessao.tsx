...
import { Check, ExternalLink, ChevronDown, ArrowLeft } from "lucide-react";
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="fixed top-0 left-0 right-0 z-[100] bg-background/90 backdrop-blur-md border-b border-border/20">
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 h-16 flex items-center justify-between">
          ...
          <span className="font-serif text-base tracking-[0.15em] text-foreground uppercase">
            Hoana Bonito
          </span>
          <div className="w-16" />
        </div>
      ...
      <main className="pt-28 pb-24 max-w-[1400px] mx-auto px-4 md:px-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-0 border-t border-border/20 mb-20 md:mb-28">
          {steps.map((step, i) => (
            <motion.div
              key={step.num}
        ...
        <motion.div
          ...
          className="border border-primary/25 bg-card"
        >
        ...
        {/* FAQ — attached below the purchase card */}
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
